<?php
    ...
    if ($islem == "sil") {
        $id = $_GET["id"];
        $sorgu = $baglan->query("delete from ek20 where (id='$id')");
        echo "<script> window.location.href='ek20.php'; </script>";
    }
    ...
    if ($islem == "ekle") {
        ...
        $text = $_POST["text"];
        $sorgu = $baglan->query("insert into ek20 (name,image,text) values ('$name','../$image','$text')");
        echo "<script> window.location.href='ek20.php'; </script>";
    }
?>
<!DOCTYPE html>
<html>
    <head>
        ...
        <?php
        $sirano = 0;
        $sorgu = $baglan->query("select * from ek20");
    ...
    <div style="text-align:center;">
        ...
        while ($satir = $sorgu->fetch_object()) {
            ...
            echo "<tr>
                    ...
                    <td>$satir->image</td>
                    <td>$satir->text</td>
                    <td><a href='ek20.php?islem=sil&id=$satir->id'>SİL</td>
                </tr>";}
        ?>
        </table><br>
        <form action="ek20.php?islem=ekle" enctype="multipart/form-data" method="post">
