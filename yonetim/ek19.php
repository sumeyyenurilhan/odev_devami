<?php
    ...
    if ($islem == "sil") {
        $id = $_GET["id"];
        $sorgu = $baglan->query("delete from ek19 where (id='$id')");
        echo "<script> window.location.href='ek19.php'; </script>";
    }
    ...
    if ($islem == "ekle") {
        ...
        $text = $_POST["text"];
        $sorgu = $baglan->query("insert into ek19 (name,image,text) values ('$name','../$image','$text')");
        echo "<script> window.location.href='ek19.php'; </script>";
    }
?>
<!DOCTYPE html>
<html>
    <head>
        ...
        <?php
        $sirano = 0;
        $sorgu = $baglan->query("select * from ek19");
    ...
    <div style="text-align:center;">
        ...
        while ($satir = $sorgu->fetch_object()) {
            ...
            echo "<tr>
                    ...
                    <td>$satir->image</td>
                    <td>$satir->text</td>
                    <td><a href='ek19.php?islem=sil&id=$satir->id'>SİL</td>
                </tr>";}
        ?>
        </table><br>
        <form action="ek19.php?islem=ekle" enctype="multipart/form-data" method="post">
