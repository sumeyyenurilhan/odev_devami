<?php
    ...
    if ($islem == "sil") {
        $id = $_GET["id"];
        $sorgu = $baglan->query("delete from ek9 where (id='$id')");
        echo "<script> window.location.href='ek9.php'; </script>";
    }
    ...
    if ($islem == "ekle") {
        ...
        $text = $_POST["text"];
        $sorgu = $baglan->query("insert into ek9 (name,image,text) values ('$name','../$image','$text')");
        echo "<script> window.location.href='ek9.php'; </script>";
    }
?>
<!DOCTYPE html>
    ...
    <div style="text-align:center;">
        ...
        while ($satir = $sorgu->fetch_object()) {
            ...
            echo "<tr>
                    ...
                    <td>$satir->image</td>
                    <td>$satir->text</td>
                    <td><a href='ek9.php?islem=sil&id=$satir->id'>SİL</td>
                </tr>";}
        ?>
        </table><br>
        <form action="ek9.php?islem=ekle" enctype="multipart/form-data" method="post">
