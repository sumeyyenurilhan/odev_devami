<?php
    ...
    if ($islem == "sil") {
        $id = $_GET["id"];
        $sorgu = $baglan->query("delete from seyahatler where (id='$id')");
        echo "<script> window.location.href='seyahatler.php'; </script>";
    }
    ...
    if ($islem == "ekle") {
        ...
        $file = "yonetim/" .$_FILES["file"]["name"];
        move_uploaded_file($_FILES["file"]["tmp_file"],$file);
        $sorgu = $baglan->query("insert into seyahatler (name,image,comment,link,file) values ('$name','../$image','$comment','../$link','../$file')");
        echo "<script> window.location.href='seyahatler.php'; </script>";
    }
?>
<!DOCTYPE html>
<html>
    <head>
        <meta charset="utf-8">
        <title>Yönetim Paneli Seyahatler</title>
    <style>
    ...
    <body>
        ...
        <table>
            <tr>
                ...
                <th>İÇERİK</th>
                <th>LİNKLER</th>
                <th>DÜZENLEME SAYFASI</th>
                <th>SİL</th>
            </tr>  
        <?php
        $sirano = 0;
        $sorgu = $baglan->query("select * from seyahatler");
        while ($satir = $sorgu->fetch_object()) {
            ...
            echo "<tr>
                ...
                <td><a href='seyahatler.php?islem=sil&id=$satir->id'>SİL</a></td>
                </tr>";}
        ?>
        </table><br>
        <form action="seyahatler.php?islem=ekle" enctype="multipart/form-data" method="post">
            <b>Başlık</b><hr><br><input type="text" name="name"><br>
            <b>Resim</b><hr><br><input type="file" name="image"><br>
            <b>İçerik</b><hr><br><input type="text" name="comment"><br>
            <b>Link</b><hr><br><input type="file" name="link"><br>
            <b>Düzenleme Linki</b><hr><br><input type="file" name="file"><br>
