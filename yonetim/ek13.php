<?php
    ...
    if ($islem == "sil") {
        $id = $_GET["id"];
        $sorgu = $baglan->query("delete from ek13 where (id='$id')");
        echo "<script> window.location.href='ek13.php'; </script>";
    }
    ...
    if ($islem == "ekle") {
        ...
        $text = $_POST["text"];
        $sorgu = $baglan->query("insert into ek13 (name,image,text) values ('$name','../$image','$text')");
        echo "<script> window.location.href='ek13.php'; </script>";
    }
?>
<!DOCTYPE html>
<html>
    <head>
        ...
        <?php
        $sirano = 0;
        $sorgu = $baglan->query("select * from ek13");
    ...
    <div style="text-align:center;">
        ...
        while ($satir = $sorgu->fetch_object()) {
            ...
            echo "<tr>
                    ...
                    <td>$satir->image</td>
                    <td>$satir->text</td>
                    <td><a href='ek13.php?islem=sil&id=$satir->id'>SİL</td>
                </tr>";}
        ?>
        </table><br>
        <form action="ek13.php?islem=ekle" enctype="multipart/form-data" method="post">
